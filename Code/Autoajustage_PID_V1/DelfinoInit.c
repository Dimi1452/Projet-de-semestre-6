/** \file DelfinoInit.c
* \brief Global functions to init the device and his peripheral 
*
...
/** \fn void DeviceInit(void) 
* \brief Init the device and the peripherals.
*
* This functions configures : 
* - the system clock
* - the peripherals clock
...
* 
* \author Texas Intrument, edit by Florian Berset & Sylvain Décastel
* \date 06.2009
*/
void DeviceInit(void)
{
...
// 0x2 =  25	MHz		(2)
	...
	PLLset(0xF);	// choose from options above based on the reference crystal
...
// HIGH / LOW SPEED CLOCKS prescale register settings
   SysCtrlRegs.HISPCP.all = 0x0002;		// old value "0x0002" Sysclk / 4 (25 MHz)
   SysCtrlRegs.LOSPCP.all = 0x0002;		// old value "0x0002" Sysclk / 4 (25 MHz)
...
// EPWM's Configuré dans PWMlib.c et DSP2833x_Epwm.c
// --------
//--------------------------------------------------------------------------------------
...
//-----------
//--------------------------------------------------------------------------------------
//  GPIO-14 - PIN FUNCTION = SCITX
	GpioCtrlRegs.GPAMUX1.bit.GPIO14 = 2;	// 0=GPIO,  1=TZ3,  2=SCITX-B,  3=MCLKX-B
...
//--------------------------------------------------------------------------------------
//  GPIO-15 - PIN FUNCTION = SCIRX
	GpioCtrlRegs.GPAMUX1.bit.GPIO15 = 2;	// 0=GPIO,  1=TZ4,  2=SCIRX-B,  3=MFSX-B
...
//--------------------------------------------------------------------------------------
//  GPIO-22 - PIN FUNCTION = QEP1S not used
...
//--------------------------------------------------------------------------------------
//  GPIO-27 - PIN FUNCTION = QEP2S not used
...
// Status LED Run and Error
//----------------------------
//--------------------------------------------------------------------------------------
//  GPIO-28 - PIN FUNCTION = Error Led
...
// I2C externe non initialisé pour le moment
//-------------
//--------------------------------------------------------------------------------------
//  GPIO-32 - PIN FUNCTION 
...
//--------------------------------------------------------------------------------------
//  GPIO-37 - PIN FUNCTION = Dout8 
	GpioCtrlRegs.GPBMUX1.bit.GPIO37 = 0;	// 0=GPIO,  1=SCIA-RX,  2=Resv,  3=Resv
...
//--------------------------------------------------------------------------------------
//  GPIO-76 - PIN FUNCTION = Dout1
	GpioCtrlRegs.GPCMUX1.bit.GPIO76 = 0;	// 0=GPIO,  1=SCIA-TX,  2=Resv,  3=Resv
...
//--------------------------------------------------------------------------------------
//  GPIO-77 - PIN FUNCTION = Dout2 
	GpioCtrlRegs.GPCMUX1.bit.GPIO77 = 0;	// 0=GPIO,  1=SCIA-RX,  2=Resv,  3=Resv
...
//--------------------------------------------------------------------------------------
//  GPIO-78 - PIN FUNCTION = Dout3 
	GpioCtrlRegs.GPCMUX1.bit.GPIO78 = 0;	// 0=GPIO,  1=SCIA-RX,  2=Resv,  3=Resv
...
//--------------------------------------------------------------------------------------
//  GPIO-79 - PIN FUNCTION = Dout4
	GpioCtrlRegs.GPCMUX1.bit.GPIO79 = 0;	// 0=GPIO,  1=Resv,  2=Resv,  3=Resv
...
//--------------------------------------------------------------------------------------
//  GPIO-49 - PIN FUNCTION = Led4 Green
...
//--------------------------------------------------------------------------------------
//  GPIO-50 - PIN FUNCTION = Led3 RED
	GpioCtrlRegs.GPBMUX2.bit.GPIO50 = 0;	// 0=GPIO,  1=ECAP5,  2=XD31,  3=Resv
...
//--------------------------------------------------------------------------------------
//  GPIO-51 - PIN FUNCTION = Led3 Green
	GpioCtrlRegs.GPBMUX2.bit.GPIO51 = 0;	// 0=GPIO,  1=ECAP6,  2=XD30,  3=Resv
...
//--------------------------------------------------------------------------------------
//  GPIO-52 - PIN FUNCTION = Led2 RED
	GpioCtrlRegs.GPBMUX2.bit.GPIO52 = 0;	// 0=GPIO,  1=ECAP5,  2=XD31,  3=Resv
...
//--------------------------------------------------------------------------------------
//  GPIO-53 - PIN FUNCTION = Led2 Green
	GpioCtrlRegs.GPBMUX2.bit.GPIO53 = 0;	// 0=GPIO,  1=ECAP6,  2=XD30,  3=Resv
...
//--------------------------------------------------------------------------------------
//  GPIO-54 - PIN FUNCTION = Led1 RED
	GpioCtrlRegs.GPBMUX2.bit.GPIO54 = 0;	// 0=GPIO,  1=ECAP5,  2=XD31,  3=Resv
...
//--------------------------------------------------------------------------------------
//  GPIO-55 - PIN FUNCTION = Led1 Green
	GpioCtrlRegs.GPBMUX2.bit.GPIO55 = 0;	// 0=GPIO,  1=ECAP6,  2=XD30,  3=Resv
...
// --------------
//--------------------------------------------------------------------------------------
//  GPIO-56 - PIN FUNCTION = Din8
	GpioCtrlRegs.GPBMUX2.bit.GPIO56 = 0;	// 0=GPIO,  1=ECAP6,  2=XD30,  3=Resv
...
//--------------------------------------------------------------------------------------
//  GPIO-57 - PIN FUNCTION = Din7
	GpioCtrlRegs.GPBMUX2.bit.GPIO57 = 0;	// 0=GPIO,  1=ECAP6,  2=XD30,  3=Resv
...
/** \brief Initializes the PLLCR register
* \author Texas Instrument 
* \param val value to set the PLLCR register
*/
...
/** \brief Reset the PIE (peripheral interrupt) control registers
* \author Texas Instrument
*/
void PieCntlInit(void)
{
...
/** \brief Initializes the PIE interrupt vector table
* \author Texas Instrument
*/
void PieVectTableInit(void)
{
...
*
* This routine is used by PieVectTableInit() to initialize the interrupt vectors.
* \author Texas Instrument
*/
...
* \author Texas Instrument
* \attention This function MUST be executed out of RAM. 
* Executing itout of OTP/Flash will yield unpredictable results
*/
void InitFlash(void)
{
   EALLOW;
...
/** Copy data from source address to source end address to dest address...
* \author Texas Instrument
* \date 2009
* \param[in]    *SourceAddr Start of source address
* \param[in]    *SourceEndAddr End of source address
* \param[out]   *DestAddr Start of destination address
* \attention No checks are made for invalid memory locations or that the end address is > then the first start address.
*/
...
/** \brief Initilizes the timer 0
* \author Texas Instrument, modified by Florian Berset
* \date 06.2009
* 
*/
void InitTimer0(void)
{
	...
	EDIS;
	InitCpuTimers();
	ConfigCpuTimer(&CpuTimer0, 150.0, Period_Interrupt);
